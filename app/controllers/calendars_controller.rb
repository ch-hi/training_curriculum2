class CalendarsController < ApplicationController
  ...
  # 予定の保存
    def create
    Plan.create(plan_params) #セットになってるキーとバリューを取り出してDBに保存
  ...
  def get_week
    ...
    7.times do |x|
      ...
      days = {month: (@todays_date + x).month, date: (@todays_date+x).day, plans: today_plans, wday: wday[wday_num]　} #配列から要素を取り出す記述が必要:wdays[4]
      @week_days.push(days)
    end
